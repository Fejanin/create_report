...
RED_FONT = 'ff0000'
RED_CELL = 'f05959'
NUM_FORMAT = '0_ ;[Red]\-0\ ' # настройка ячеек: чило без десятых, отрицательные красным цветом
HEADER_COLOR = 'FFF4C5'
...
def create_base_table_from_dv_file(file_dv):
    ...
    for row in range(1, max_row + 1):
        ...
        if ws.cell(row=row, column=1).value == 'Основной склад Останкино':
            flag = True
        if ws.cell(row=row + 1, column=1).value == 'Основной склад сыров':
            flag = False
    return table
            ...
            if not all(all_columns.values()):
                ...
                if cell_value == 'ср':
                    if ws.cell(row=row + 1, column=col).value:
                        ...
                        columns_dict.append(col)
                if cell_value in ('комментарии', 'коментарии'):
...
def fill_file(table, ws, a_c):
    ...
    for row, obj_row in enumerate(rows, table.START_ROWS):
        ...
        cell_border = a_c[table.COLUMNS['krim_order'][1] - 1]
        ws[f'{cell_border}{row}'].border = Border(top=thick, left=thick, right=thick, bottom=thick)
